//! Differential test of `change_of_basis_matrix_tate`.

mod common;

use sqisign_id2iso::change_of_basis_matrix_tate;
use sqisign_quaternion::dim2::ibz_mat_2x2_new;
use sqisign_vectors::load;

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/id2iso/change_of_basis_matrix_tate.json"
);

#[test]
fn change_of_basis_matrix_tate_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_id2iso::change_of_basis_matrix_tate");
    ...
    for v in &f.vectors {
        ...
        change_of_basis_matrix_tate(&mut mat, &b1, &b2, &mut e, fparam);
        assert!(
