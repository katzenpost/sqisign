//! Differential test of the ported `multiple_mp_shiftl` against the
//! committed C-derived vectors. The battery includes shifts straddling
//! RADIX and exceeding the full bit width (result all zero).
...
const VECTORS: &str = concat!(
    ...
    "/../../vectors/mp/multiple_mp_shiftl.json"
);
...
#[test]
fn multiple_mp_shiftl_matches_reference_vectors() {
    let file = load(VECTORS).expect("vector file must load, be canonical, and match the pin");
    assert_eq!(file.boundary, "sqisign_mp::multiple_mp_shiftl");
    assert!(
    ...
    for v in &file.vectors {
        ...
        assert_eq!(
            x, expected,
            "vector {} diverged from the C reference (shift={})",
            v.id, shift
        );
    }
}
