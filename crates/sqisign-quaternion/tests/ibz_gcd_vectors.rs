//! Differential test of `ibz_gcd` against committed C-derived vectors.
use sqisign_quaternion::{ibz_gcd, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_gcd.json"
);
...
#[test]
fn ibz_gcd_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_gcd");
