//! Differential test of `quat_alg_conj`.
use sqisign_quaternion::{quat_alg_conj, Ibz, QuatAlgElem};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/quat_alg_conj.json"
);
...
#[test]
fn quat_alg_conj_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::quat_alg_conj");
    assert!(f.vectors.len() >= 50);
    for v in &f.vectors {
        let x = read_elem("x", &v.inputs);
        let exp = read_elem("r", &v.outputs);
        let mut r = QuatAlgElem::new();
        quat_alg_conj(&mut r, &x);
