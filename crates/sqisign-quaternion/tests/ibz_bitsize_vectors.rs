//! Differential test of `ibz_bitsize`.
use sqisign_quaternion::{ibz_bitsize, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_bitsize.json"
);
...
#[test]
fn ibz_bitsize_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_bitsize");
    assert!(f.vectors.len() >= 200);
    for v in &f.vectors {
        let a = read_ibz("a", &v.inputs["a"]);
        let exp = read_le_i32("r", &v.outputs["r"]);
        let got = ibz_bitsize(&a);
