//! Differential test of `ibz_sub`.
use sqisign_quaternion::{ibz_sub, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_sub.json"
);
...
#[test]
fn ibz_sub_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_sub");
