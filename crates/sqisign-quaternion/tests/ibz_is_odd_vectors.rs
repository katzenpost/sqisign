//! Differential test of `ibz_is_odd` against committed C-derived vectors.
use sqisign_quaternion::{ibz_is_odd, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_is_odd.json"
);
...
#[test]
fn ibz_is_odd_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_is_odd");
    assert!(f.vectors.len() >= 200);
    for v in &f.vectors {
        let a = read_ibz("a", &v.inputs["a"]);
        let exp = read_i8("r", &v.outputs["r"]);
        let got = ibz_is_odd(&a);
