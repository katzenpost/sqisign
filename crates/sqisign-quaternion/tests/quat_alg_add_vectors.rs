//! Differential test of `quat_alg_add`.
mod common;
use common::{ibz_eq, read_ibz, vec4_eq};
use sqisign_quaternion::{quat_alg_add, QuatAlgElem};
use sqisign_vectors::load;

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/quat_alg_add.json"
);
...
#[test]
fn quat_alg_add_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::quat_alg_add");
