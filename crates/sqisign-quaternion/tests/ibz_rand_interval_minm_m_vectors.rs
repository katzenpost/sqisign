//! Differential test of `ibz_rand_interval_minm_m` against C-derived vectors.

mod common;

use common::{read_i32, read_ibz};
use sqisign_common::CtrDrbg;
use sqisign_quaternion::{ibz_rand_interval_minm_m, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_rand_interval_minm_m.json"
);
...
#[test]
fn ibz_rand_interval_minm_m_matches_reference_vectors() {
    let f = load(VECTORS).expect("vector file load");
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_rand_interval_minm_m");
