//! Differential test of `quat_alg_scalar`.
use sqisign_quaternion::{quat_alg_scalar, Ibz, QuatAlgElem};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/quat_alg_scalar.json"
);
...
#[test]
fn quat_alg_scalar_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::quat_alg_scalar");
    assert!(f.vectors.len() >= 20);
    for v in &f.vectors {
        let num = read_ibz("num", &v.inputs["num"]);
        let den = read_ibz("den", &v.inputs["den"]);
        let exp = read_elem("r", &v.outputs);
        let mut r = QuatAlgElem::new();
        quat_alg_scalar(&mut r, &num, &den);
