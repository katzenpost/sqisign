//! Differential test of `ibz_divides` against committed C-derived vectors.
use sqisign_quaternion::{ibz_divides, Ibz};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/ibz_divides.json"
);
...
#[test]
fn ibz_divides_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::ibz_divides");
    assert!(f.vectors.len() >= 1000);
    for v in &f.vectors {
        let a = read_ibz("a", &v.inputs["a"]);
        let b = read_ibz("b", &v.inputs["b"]);
        let exp = read_i8("r", &v.outputs["r"]);
        let got = ibz_divides(&a, &b);
