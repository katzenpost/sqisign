//! Differential test of `quat_alg_elem_copy_ibz`.
use sqisign_quaternion::{quat_alg_elem_copy_ibz, Ibz, QuatAlgElem};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/quaternion/quat_alg_elem_copy_ibz.json"
);
...
#[test]
fn quat_alg_elem_copy_ibz_matches_reference_vectors() {
    let f = load(VECTORS).unwrap();
    assert_eq!(f.boundary, "sqisign_quaternion::quat_alg_elem_copy_ibz");
    assert!(f.vectors.len() >= 50);
    for v in &f.vectors {
        let denom = read_ibz("denom", &v.inputs["denom"]);
        let c0 = read_ibz("c0", &v.inputs["c0"]);
        let c1 = read_ibz("c1", &v.inputs["c1"]);
        let c2 = read_ibz("c2", &v.inputs["c2"]);
        let c3 = read_ibz("c3", &v.inputs["c3"]);
        let exp = read_elem("r", &v.outputs);
        let mut r = QuatAlgElem::new();
        quat_alg_elem_copy_ibz(&mut r, &denom, &c0, &c1, &c2, &c3);
