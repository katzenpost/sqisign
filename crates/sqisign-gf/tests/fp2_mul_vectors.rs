//! Differential test of the ported `fp2_mul` against the committed
//! C-derived vectors. Binary shape: (a_re, a_im, b_re, b_im) ->
//! (c_re, c_im). Karatsuba-style three-multiplication scheme;
//! statement order mirrors `fp2.c:95..107` exactly.

use sqisign_gf::{fp2_mul, Fp, Fp2, NWORDS_FIELD};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../vectors/gf/fp2_mul.json");

fn fp_from(label: &str, bytes: &[u8]) -> Fp {
    assert_eq!(
        bytes.len(),
        NWORDS_FIELD * 8,
        "{label} must be exactly {NWORDS_FIELD} u64 limbs"
    );
    let mut limbs = [0u64; NWORDS_FIELD];
    for (i, chunk) in bytes.chunks_exact(8).enumerate() {
        limbs[i] = u64::from_le_bytes(chunk.try_into().unwrap());
    }
    limbs
}

#[test]
fn fp2_mul_matches_reference_vectors() {
    let file = load(VECTORS).expect("vector file must load, be canonical, and match the pin");
    assert_eq!(file.boundary, "sqisign_gf::fp2_mul");
    assert!(file.vectors.len() >= 1000);

    for v in &file.vectors {
        let a_re = fp_from(
            "a_re",
            &decode("a_re", &v.inputs["a_re"]).expect("a_re hex"),
        );
        let a_im = fp_from(
            "a_im",
            &decode("a_im", &v.inputs["a_im"]).expect("a_im hex"),
        );
        let b_re = fp_from(
            "b_re",
            &decode("b_re", &v.inputs["b_re"]).expect("b_re hex"),
        );
        let b_im = fp_from(
            "b_im",
            &decode("b_im", &v.inputs["b_im"]).expect("b_im hex"),
        );
        let exp_re = fp_from(
            "c_re",
            &decode("c_re", &v.outputs["c_re"]).expect("c_re hex"),
        );
        let exp_im = fp_from(
            "c_im",
            &decode("c_im", &v.outputs["c_im"]).expect("c_im hex"),
        );

        let a = Fp2 { re: a_re, im: a_im };
        let b = Fp2 { re: b_re, im: b_im };
        let mut c = Fp2 {
            re: [0u64; NWORDS_FIELD],
            im: [0u64; NWORDS_FIELD],
        };
        fp2_mul(&mut c, &a, &b);
        assert_eq!(c.re, exp_re, "vector {} diverged on c_re", v.id);
        assert_eq!(c.im, exp_im, "vector {} diverged on c_im", v.id);
    }
}
