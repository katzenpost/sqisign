//! Differential test of the ported `fp2_inv` against the committed
//! C-derived vectors. Unary shape: (a_re, a_im) -> (c_re, c_im).
//!
//! `fp2_inv(x)` is in-place; the cdump harness uses the
//! `fp2_inv_adapter` to snapshot the input as `a` and the post-call
//! buffer as `c`. The replay copies the recorded inputs into a fresh
//! `Fp2` and reads the post-call limbs.

use sqisign_gf::{fp2_inv, Fp, Fp2, NWORDS_FIELD};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../vectors/gf/fp2_inv.json");
