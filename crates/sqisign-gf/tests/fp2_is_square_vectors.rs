//! Differential test of the ported `fp2_is_square` against the
//! committed C-derived vectors. Unary predicate shape:
//! (a_re, a_im) -> result (4-byte LE u32 mask).
//!
//! `fp2_is_square(a)` uses the norm reduction `is_square(re^2 + im^2)`;
//! roughly half of random inputs are squares in `Fp^2`, so both
//! outcomes are well-represented in the sweep.

use sqisign_gf::{fp2_is_square, Fp, Fp2, NWORDS_FIELD};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(
    env!("CARGO_MANIFEST_DIR"),
    "/../../vectors/gf/fp2_is_square.json"
);
...
#[test]
fn fp2_is_square_matches_reference_vectors() {
    let file = load(VECTORS).expect("vector file must load, be canonical, and match the pin");
    assert_eq!(file.boundary, "sqisign_gf::fp2_is_square");
    ...
    for v in &file.vectors {
        ...
        let got = fp2_is_square(&a);
        assert_eq!(
            got, expected,
            "vector {} diverged from the C reference at the fp2_is_square result mask",
            v.id
        );
        match expected {
            0xFFFF_FFFF => pos += 1,
