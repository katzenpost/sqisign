//! Differential test of the ported `fp2_add` against the committed
//! C-derived vectors. Binary shape: (a_re, a_im, b_re, b_im) ->
//! (c_re, c_im). Componentwise `fp_add` on both halves.

use sqisign_gf::{fp2_add, Fp, Fp2, NWORDS_FIELD};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../vectors/gf/fp2_add.json");
