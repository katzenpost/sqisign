//! Differential test of the ported `fp2_neg` against the committed
//! C-derived vectors. Unary shape: (a_re, a_im) -> (c_re, c_im).
//! `fp2_neg(c, a)` is componentwise `fp_neg` on both halves.

use sqisign_gf::{fp2_neg, Fp, Fp2, NWORDS_FIELD};
use sqisign_vectors::{decode, load};

const VECTORS: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/../../vectors/gf/fp2_neg.json");
