...
| `sqisign-gf` | `fp_sqr` | `fp_sqr(a) == fp_mul(a, a)` bit-exact for arbitrary five-limb inputs (the squaring optimisation's column sums equal `modmul`'s column sums and `u128` wrapping_add is associative and commutative), structural carry invariant (limbs 0..=3 < 2^51; limb 4 fully unmasked by design, the reference's final `c[4] = (spint)t` is a 64-bit truncation) | link `fp_p5248_64.c` for byte-equality vs C |
| `sqisign-gf` | `fp_half` | `fp_half(a) == fp_mul(&TWO_INV, a)` bit-exact for arbitrary five-limb inputs (the port is literally that one `modmul` call, so the cross-oracle is a sanity tautology), structural carry invariant (limbs 0..=3 < 2^51; limb 4 fully unmasked by design, inherited from `modmul`'s `c[4] = (spint)t` truncation) | link `fp_p5248_64.c` for byte-equality vs C |
| `sqisign-gf` | `fp_div3` | `fp_div3(a) == fp_mul(&THREE_INV, a)` bit-exact for arbitrary five-limb inputs (the port is literally that one `modmul` call, so the cross-oracle is a sanity tautology), structural carry invariant (limbs 0..=3 < 2^51; limb 4 fully unmasked by design, inherited from `modmul`'s `c[4] = (spint)t` truncation) | link `fp_p5248_64.c` for byte-equality vs C |
...
| `sqisign-gf` | `fp_set_small` | `val == 0` produces the canonical all-zero limb vector; `val == 1` produces the Montgomery `ONE` constant bit-exact and equal to `fp_set_one(_)`; high-bits-ignored narrowing `fp_set_small(out, val) == fp_set_small(out, val as i32 as u64)` for arbitrary `val` (only the low 32 bits sign-extended are observable through the boundary) | link `fp_p5248_64.c` for byte-equality vs C |
| `sqisign-gf` | `fp_mul_small` | `val == 0` produces the canonical all-zero limb vector for arbitrary `a`; `val == 1` produces the same redundant representative as `fp_mul(a, &MONTGOMERY_ONE)` bit-exact; cross-oracle `fp_mul_small(out, a, val) == fp_mul(a, &fp_set_small(val as i32 as u64))` bit-exact (the port is the literal `modint + modmul` chain `modmli` evaluates); structural carry invariant (limbs 0..=3 < 2^51; limb 4 fully unmasked by design, inherited from `modmul`'s `c[4] = (spint)t` truncation) | link `fp_p5248_64.c` for byte-equality vs C |
...
| `sqisign-gf` | `fp_cswap` | LSB-only conditional swap: `ctl & 1 == 0` leaves both operands unchanged, `ctl & 1 == 1` exchanges them limb for limb; double cswap with the same `ctl` is the identity; only the LSB of `ctl` is consulted, so high bits never alter the result | link `fp_p5248_64.c` (lvl1) for byte-equality vs C |
| `sqisign-gf` | `fp_is_zero` | returned `u32` is always `0` or `0xFFFFFFFF` (the C `-(uint32_t)int01` invariant on the inner `modis0` `{0, 1}` result), `fp_is_zero` of the canonical all-zero limb pattern is the all-ones mask | link `fp_p5248_64.c` (lvl1) for byte-equality vs C |
| `sqisign-gf` | `fp_is_equal` | returned `u32` is always `0` or `0xFFFFFFFF` (the C `-(uint32_t)int01` invariant on the inner `modcmp` `{0, 1}` result), reflexivity `fp_is_equal(a, a) == 0xFFFFFFFF`, symmetry `fp_is_equal(a, b) == fp_is_equal(b, a)` bit-exact | link `fp_p5248_64.c` (lvl1) for byte-equality vs C |
